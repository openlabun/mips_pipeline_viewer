...
const calculateNextState = (currentState: SimulationState): SimulationState => {
    ...
    if (
        ...
        if (stageIndex == 0 && currentState.instructions[index - 1]) {
          ...
          if (currentState.instructions[index - 2]) {
            ...
            if (
              ...
              if (
                Stall(i2, i1) &&
                (currentState.instructionStages[index - 1] ?? -1) + 2 <
                  currentState.stageCount &&
      ...
      //FW logic
        ...
        if (stageIndex == 1 && currentState.instructions[index - 1]) {
          ...
          if (
            canForward(i2, i1)[0] &&
            (currentState.instructionStages[index - 1] ?? -1) + 3 <
              currentState.stageCount &&
            !currentState.instructionFinished[index - 1]
          ) {
            //console.log("posible");
          ...
          if (!canForward(i2, i1)[1]) {
            ...
            newInstructionStages[index] =
              (currentState.instructionStages[index] ?? 1) <= 1
                ? 1
                : currentState.instructionStages[index];
            activeInstructions++;
          }
        } else {
  ...
  const completionCycle =
    currentState.instructions.length > 0
      ...
      : 0;
  const isFinished =
    currentState.instructionFinished[currentState.instructions.length - 1];
